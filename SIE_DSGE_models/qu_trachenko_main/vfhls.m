function V = vfhls(theta1,theta2,msel1,msel2)
%computes asymptotic variance for LR test for LS(2004)-type models
...
vint=0; %initialize the quantity inside the integral
for i=1:n
    
    temp = (eye(dimy) - (sp_mat1(((i-1)*dimy+1):(i*dimy),:)/sp_mat2(((i-1)*dimy+1):(i*dimy),:)))^2; %(I-f_theta*inv(h_phi))
    
    vint = vint + trace(temp); %term inside the integral
end

V = real((1/(4*pi))*(2*pi/n)*vint); %final result. Take real part since very small complex residual may remain.
